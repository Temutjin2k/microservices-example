...
import (
	...
	"net/http"
	"net/url"
	"order_service/internal/adapter/http/myrouter/invdto"
...
func (r *InventoryRouter) GetById(id int64) (model.Inventory, error) {
	...
	if err != nil {
		return model.Inventory{}, fmt.Errorf("HTTP request failed: %v", err)
	}
	defer resp.Body.Close()

	// Check the status code
	if resp.StatusCode != http.StatusOK {
		return model.Inventory{}, fmt.Errorf("unexpected status code: %d", resp.StatusCode)
	}
	...
	if err != nil {
		return model.Inventory{}, fmt.Errorf("failed to parse response: %v", err)
	}
