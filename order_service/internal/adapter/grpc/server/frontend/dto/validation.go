...
func ValidateSetOrderStatusRequest(v *validator.Validator, req *orderpb.OrderSetStatusRequest) {
	safeList := []string{dao.OrderStatusCanceled, dao.OrderStatusCompleted, dao.OrderStatusPending}
	v.Check(validator.PermittedValue(req.GetStatus(), safeList...), "status", fmt.Sprintf("invalid status. Available: %v", strings.Join(safeList, ", ")))
}
