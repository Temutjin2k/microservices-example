...
func (r *Order) GetListWithFilter(ctx context.Context, filter model.OrderFilter) ([]model.Order, error) {
	...
	itemsMap := make(map[int64][]model.OrderItem)
	for itemRows.Next() {
		var item model.OrderItem
	...
	for i, order := range orders {
		if items, ok := itemsMap[order.ID]; ok {
			...
			orders[i].OrderItems = []model.OrderItem{} // empty slice if no items
		}
	}
...
func (r *Order) Update(ctx context.Context, update model.OrderUpdateData) error {
	tx, err := r.db.Begin(ctx)
	if err != nil {
		return err
	}
