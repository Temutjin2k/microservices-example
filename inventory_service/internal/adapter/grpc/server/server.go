...
func New(cfg config.GRPCServer, userUsecase InventoryUsecase) *API {
	return &API{
		cfg:              cfg,
		addr:             fmt.Sprintf("0.0.0.0:%d", cfg.Port),
		inventoryUsecase: userUsecase,
	}
}
...
func (a *API) run(ctx context.Context) error {
	...
	inventorypb.RegisterInventoryServiceServer(a.s, frontend.New(a.inventoryUsecase))
